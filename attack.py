...
class DynamicsToolkit:
    ...
    def extract_features(self, loss_seq: List[float]) -> Dict[str, float]:
        ...
        if len(descending_steps) > 0:
            ecd = np.sum(np.abs(descending_steps))
        else:
            ecd = 0.0
        ...
        if len(y_tail) >= 2:
            tf = np.sum(np.abs(np.diff(y_tail)))
        else:
            tf = 0.0
...
class Phase2Investigator:
    ...
    def compute_empirical_percentile(self, val: float, key: str) -> float:
        ...
        percentile = np.searchsorted(bg_array, val, side="right") / len(bg_array)
        return float(percentile)

    def analyze(self, sample: Sample, vlm_score: float, adaptive_lambda: float) -> Tuple[float, Dict]:
        phy = self.physics.extract_features(sample.loss_sequence)

        p_ecd = 1.0 - self.compute_empirical_percentile(phy['ecd'], 'ecd')
        p_tf = 1.0 - self.compute_empirical_percentile(phy['tf'], 'tf')
        score_phy = 0.5 * p_ecd + 0.5 * p_tf

        final_score = adaptive_lambda * score_phy + 1.0 * vlm_score
...
class LiraLiteAuditor:
    def __init__(self, dataset: str, model: str, vlm_type: str = "qwen3_2b", vlm_path: str | None = None,
                 ...
                 lambda_cap: float = 1.0, calib_threshold: float = 0.1,
        ...
        self.lambda_cap = min(1.0, float(lambda_cap))
    ...
    def run(self):
        ...
        adaptive_lambda = (1.0 - mean_confidence) / (mean_confidence + 1e-5)
        adaptive_lambda = min(self.lambda_cap, float(adaptive_lambda))
        ...
        if self.enable_phase2:
            ...
            for sample in tqdm(self.state.samples):
                ...
                if vlm_score < self.calib_threshold:
                    ...
                    final_score, phy = investigator.analyze(
                        sample, vlm_score, adaptive_lambda=adaptive_lambda
                    )
...
def run_attack(dataset: str, model: str, max_samples: int = 1000,
    ...
    auditor = LiraLiteAuditor(
        ...
        lambda_cap=lambda_cap, calib_threshold=calib_threshold,
